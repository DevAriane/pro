...
let somme;
  ...
  const { imageUrl, nutritionInfo, price, customizationOptions, id, isAvailable, isPopular, description, name, restaurantId } = item;
  ...
  return (
    <SafeAreaView style={styles.area}>
      ...
      <View style={styles.containt}>
        ...
        <ScrollView>
          <View ><Image source={imageUrl} style={{ width: 400, height: 200 }} resizeMode="cover" /></View>
          <View style={{ display: 'flex', justifyContent: 'flex-start', margin: 10 }}>
            ...
            <View>
              <Text style={{ color: 'green', fontWeight: 'bold', fontSize: 18 }}>${price}</Text>
            </View>
          </View>
          <View style={{ margin: 10 }}>
            ...
            <Text>Is Available: {isAvailable.toString()}</Text> <Text>Is Popular: {isPopular.toString()}</Text>
            {customizationOptions.map((x, index) =>
          
            (<View key={index}> <Text>{x.name}</Text>
          
              {
              x.options.map((a, i) =>
              (<View key={i} style={styles.radio}>
                <RadioButton key={i} value={a.name} status={click[x.name] === a.name ? 'checked' : 'unchecked'} 
                onPress={() => handleRadioPress(x.name,a.name,a.priceModifier)} style={styles.radioButton} />
                <Text>{a.name}</Text> 
